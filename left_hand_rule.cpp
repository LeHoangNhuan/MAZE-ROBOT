...
// ===== THRESHOLD =====
#define FRONT_BLOCKED_MM  170//160

// Hysteresis for side walls
#define SIDE_OPEN_HIGH    270 //250   // mở khi > 230
#define SIDE_OPEN_LOW     210   // đóng khi < 220

// HARD BLOCK threshold for 180-turn
#define SIDE_BLOCKED_MM   170

// dừng 1s rồi mới quyết định
#define STOP_WAIT_MS     10
...
static WallStateL detectWalls(uint16_t fl, uint16_t fr, uint16_t left, uint16_t right) {
  ...
  Serial.print("[WALL-L] F:"); Serial.print(front);
...
static void leftHandDecision(const WallStateL &w) {
  if (pendingTurn == 1) { spinLeft90();  turnLeft();  pendingTurn = 0; moveActive = true; return; }
  if (pendingTurn == 2) { spinRight90(); turnRight(); pendingTurn = 0; moveActive = true; return; }

  bool frontClear = !w.frontBlocked;

  // Ưu tiên trái: có đường trái -> đi thẳng 1 ô rồi rẽ trái
  if (w.leftOpen) { pendingTurn = 1; goForward(); moveActive = true; return; }

  // Không có đường trái -> nếu đi thẳng được thì đi thẳng
  if (frontClear) { goForward(); moveActive = true; return; }

  // Không đi thẳng được -> nếu phải mở thì đi thẳng 1 ô rồi rẽ phải
  if (w.rightOpen) { pendingTurn = 2; goForward(); moveActive = true; return; }
...
static bool isDeadEnd(const WallStateL &w) {
  return w.frontBlocked && !w.rightOpen && !w.leftOpen;
}

static bool handleDeadEnd(const WallStateL &w) {
  bool frontClear = !w.frontBlocked;
...
void leftHandUpdate(uint16_t fl, uint16_t fr, uint16_t left, uint16_t right) {
  ...
  WallStateL w = detectWalls(fl, fr, left, right);
